...
df.drop(["distance", "elapsedDuration"], axis=1, inplace=True)
...
def _(n, md):
    ...
    fig.update_traces(
        marker=dict(symbol="circle", line=dict(width=2, color="Black")),
        selector=dict(mode="markers"),
    )
    return fig, msg


@app.callback(Output("activity_info", "children"), Input("plot_running", "clickData"))
def _(clickData):
    if not clickData:
